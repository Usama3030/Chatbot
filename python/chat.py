...
# # CONFIG
# # CSV_DATA_FILE = "./assets/data.csv"
# # CSV_DATA_FILE = "./assets/Inspection_Clean_Data.csv"
# CSV_DATA_FILE = "./assets/Incidents_Clean_Data.csv"
# DB_FILE = "data.db"
# TABLE_NAME = os.path.splitext(os.path.basename(CSV_DATA_FILE))[0].lower()

# os.environ["GROQ_API_KEY"] = "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"
...
#     raise FileNotFoundError(f"{CSV_DATA_FILE} not found!")

# if CSV_DATA_FILE.endswith(".csv"):
#     df = pd.read_csv(CSV_DATA_FILE)
# elif CSV_DATA_FILE.endswith((".xls", ".xlsx")):
#     df = pd.read_excel(CSV_DATA_FILE, engine="openpyxl")
# else:
...
# # Get sample unique values for categorical columns (for better SQL generation)
# CATEGORY_VALUES = {}
# for col in COLUMNS:
#     unique_vals = df[col].dropna().unique()
#     if len(unique_vals) < 100:  # Only store if reasonable number
#         CATEGORY_VALUES[col] = list(unique_vals)[:20]  # Limit to 20 samples
...
#     """
#     if re.search(r"\W", col_name):  # Non-alphanumeric character
#         return f"[{col_name}]"
#     return col_name

# # FUZZY MATCH for category values
...
#     Returns the exact value from data or None if no good match.
#     """
#     if column_name not in CATEGORY_VALUES:
#         return None
    
#     possible_values = CATEGORY_VALUES[column_name]
...
#     # Try fuzzy match on normalized strings
...
# # ENHANCED SQL GENERATION with category awareness
# # def generate_sql(user_question, table_name=TABLE_NAME, columns=COLUMNS):
# #     # Build column info with sample values
# #     column_info = []
# #     for col in columns:
# #         if col in CATEGORY_VALUES:
# #             samples = CATEGORY_VALUES[col][:5]  # Show 5 sample values
# #             column_info.append(f"{col} (examples: {', '.join(map(str, samples))})")
# #         else:
...
# # 2. Use EXACT column values as shown in examples (including slashes, spaces, hyphens)
# # 3. Use LIKE with wildcards for partial text matching: WHERE column LIKE '%value%'
# # 4. For categories, use the EXACT format from examples (e.g., "Slip/Trip/Fall" not "Slip Trip")
# # 5. Do NOT explain anything
# # 6. Return SQL query ONLY
# # 7. Do NOT include markdown code blocks or backticks

# # Example patterns:
# # - "near miss" → WHERE column LIKE '%Near Miss%'
# # - "slip trip" → WHERE column LIKE '%Slip/Trip%' or '%Slip%Trip%'
# # """
...
# def generate_sql(user_question, table_name=TABLE_NAME, columns=COLUMNS):
#     # Wrap table name too if it has spaces
#     safe_table_name = safe_column(table_name)
...
# # PRE-PROCESS QUESTION to replace fuzzy terms with exact values
# def preprocess_question(question: str) -> str:
#     """
#     Replace fuzzy category terms in question with exact database values.
#     Example: "slip trip" → "Slip/Trip/Fall"
#     """
#     processed = question
...
#         for col in CATEGORY_VALUES.keys():
...
#         if df.empty:
#             return jsonify({"error": "No CSV/Excel data loaded"}), 500
...
#         "table": TABLE_NAME,
#         "columns": COLUMNS,
#         "category_samples": CATEGORY_VALUES,
#         "rows": len(df)
#     }), 200
...
#     return jsonify(CATEGORY_VALUES), 200
